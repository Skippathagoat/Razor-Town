...
let W = null;
function attach(world) { W = world; ensureTables(); }
...
function sys(p) {
  ...
  if (!s.today) s.today = { day: dayKey(), crimes: 0, wins: 0, fish: 0, posts: 0, gigs: 0, races: 0, arcade: 0 };
...
function weatherNow() {
  ...
  return { ...kind, hour: d.getUTCHours(), night: d.getUTCHours() >= 20 || d.getUTCHours() < 6 };
}
...
function ensureChallenges(p) {
  ...
  if (s.challenges.day !== day) {
    ...
    s.today = { day, crimes: 0, wins: 0, fish: 0, posts: 0, gigs: 0, races: 0, arcade: 0, spins: 0, deposits: 0 };
  }
  return s;
}
...
function carRating(owned) { const c = C.CARS.find(x => x.id === owned.id); return c.spd * 0.62 + c.grp * 0.38; }
...
module.exports = {
  ...
  gigDo, courierTake, courierDeliver, fishCast, salvageRun, plasmaDonate, trialJoin, buskPlay,
