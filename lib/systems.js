...
const T26 = require('./game/t26.js');   // warrant feats watch the heat (t26 never requires this module)
...
function carRating(owned) { const c = C.CARS.find(x => x.id === owned.id) || { spd: 0, grp: 0 }; return ((c.spd || 0) + (owned.spd || 0)) * 0.62 + ((c.grp || 0) + (owned.grp || 0)) * 0.38; }
...
function turfRelease(accId, districtId) {
  ...
  const did = W.safeId(districtId);
  if (did == null) return err('That block is not yours to drop.');
  const row = db().prepare('SELECT * FROM turf WHERE district=?').get(did);
  if (!row || row.acc_id !== accId) return err('That block is not yours to drop.');
  db().prepare('DELETE FROM turf WHERE district=?').run(did);
  W.save(accId, p);
  return { ok: true, p: W.publicView(p), res: { district: did } };
}
...
// Ten families of underworld work. Each family has its own resolver, its own consequence table
// and its own interaction with street heat, the informant tips, the economy dials and the garage.
const OPS = require('./game/ops.js');
const clampN = (n, a, b) => M.max(a, M.min(b, n));

function ops(p) {
  const s = sys(p);
  if (!s.ops || typeof s.ops !== 'object') s.ops = {};
  const o = s.ops;
  if (!o.rackets || typeof o.rackets !== 'object') o.rackets = {};
  if (!o.chains || typeof o.chains !== 'object') o.chains = {};
  if (!o.cool || typeof o.cool !== 'object') o.cool = {};
  if (!o.docs || typeof o.docs !== 'object') o.docs = {};
  if (!o.fam || typeof o.fam !== 'object') o.fam = {};
  if (!Array.isArray(o.art)) o.art = [];
  if (!Number.isFinite(o.total)) o.total = 0;
  if (!Number.isFinite(o.wins)) o.wins = 0;
  if (!Number.isFinite(o.fails)) o.fails = 0;
  if (!Number.isFinite(o.earned)) o.earned = 0;
  if (!Number.isFinite(o.spent)) o.spent = 0;
  return o;
}
function opStatBonus(p, op) {
  const v = E.eff ? E.eff(p, op.stat) : ((p.stats && p.stats[op.stat]) || 0);
  return clampN(Math.round((v || 0) / 24), 0, 20);
}
function opRunWeatherBonus(w) { return ({ fog: 8, storm: 6, rain: 4, wind: 2, heat: -3, clear: -2 })[w] || 0; }
function opChance(p, op, extra) {
  const d = dials();
  let ch = op.chance + opStatBonus(p, op) + tipPct(p, 'edge');
  if (op.special === 'chain') ch += tipPct(p, 'crew');
  if (op.special === 'muscle') ch += M.round(tipPct(p, 'muscle') * 0.5);
  if (op.special === 'run') ch += opRunWeatherBonus(weatherNow().id);
  const heat = heatOf(p);
  ch -= M.round(heat * (op.special === 'run' ? 0.22 : 0.12));
  ch -= M.round((d.danger - 100) * 0.06);
  return M.round(clampN(ch + (extra || 0), 5, 97));
}
function opPay(p, op, mult) {
  const base = op.min + rnd(M.max(1, op.max - op.min + 1));
  const d = dials();
  return M.round(base * (d.payout / 100) * (1 + tipPct(p, 'payoff') / 100) * (mult || 1));
}
function opTip(p, kind, strength, durMin, from) {
  const st = sys(p);
  st.tips[kind] = { s: strength, until: now() + M.max(1, durMin) * MIN, label: TIP_LABEL[kind] || kind, from: from || 'the street' };
  return st.tips[kind];
}
// Sirens on your back: heat climbs while you work and cools on its own.
function heatOf(p) {
  const s = sys(p);
  if (!s.heat || typeof s.heat !== 'object') s.heat = { n: 0, at: now() };
  const drop = M.floor((now() - (s.heat.at || now())) / (3 * MIN));
  if (drop > 0) { s.heat.n = M.max(0, (s.heat.n || 0) - drop); s.heat.at = now(); }
  return s.heat.n || 0;
}
function addHeat(p, n) {
  const s = sys(p);
  const cur = heatOf(p);
  s.heat = { n: M.min(100, cur + n), at: now() };
  // 2026.5: crossing a warrant line is the moment the feat watches for
  try { if (T26 && T26.checkWarrantFeat) T26.checkWarrantFeat(p); } catch (e) {}
  return s.heat.n;
}
function coolHeat(p, n) {
  const s = sys(p);
  const cur = heatOf(p);
  s.heat = { n: M.max(0, cur - n), at: now() };
  return s.heat.n;
}
function opHeat(p, n) { addHeat(p, M.max(1, M.round(n * (dials().danger / 100)))); }
// the danger dial decides how often a bad night ends in a cell instead of a story
function opDanger(p, op, scale) {
  const chance = (0.09 + op.grade * 0.013) * (scale || 1) * (dials().danger / 100);
  if (Math.random() < chance) {
    const mins = 5 + rnd(16) + op.grade * 3;
    p.jail_until = M.max(p.jail_until || 0, now() + mins * MIN);
    return `The desk makes it official — ${mins} minutes in a cell.`;
  }
  return '';
}
function opLocked(p, op) {
  if ((p.level || 1) < op.level) return `Needs level ${op.level}.`;
  if (op.crew && !(p.faction || p.gang) ) return `Needs a crew behind you (${op.crew} strong).`;
  return null;
}
function opCooldown(p, id) { const o = ops(p); return M.max(0, (o.cool[id] || 0) - now()); }
function opsSetCool(p, id, mins) { ops(p).cool[id] = now() + mins * MIN; }
function opRacket(p, op) {
  const o = ops(p);
  const R = o.rackets[op.id];
  if (!R) return null;
  const per = op.hours * HOUR;
  const cap = op.capHours * HOUR;
  const elapsed = M.max(0, now() - (R.at || now()));
  const cycles = M.min(op.capHours / op.hours, elapsed / per);
  const value = M.floor((op.min + (op.max - op.min) * 0.5) * cycles);
  return { id: op.id, name: op.name, ico: op.ico, at: R.at, hours: op.hours, cycles: M.floor(cycles * 10) / 10, value,
    full: elapsed >= cap, rate: M.floor((op.min + op.max) / 2), nextIn: M.max(0, per - (elapsed % per)) };
}
function opArtValue(piece, at) {
  const hours = M.max(0, ((at || now()) - piece.at) / HOUR);
  const grown = piece.paid * (1 + 0.065 * hours);
  return M.round(M.min(grown, piece.paid * 3.2), 50);
}
function opView(p, op) {
  const o = ops(p);
  const own = o.rackets[op.id] || null;
  const chain = o.chains[op.id] || null;
  return {
    ...op,
    live: opChance(p, op),
    locked: opLocked(p, op),
    cool: opCooldown(p, op.id),
    owned: !!own,
    rate: own ? M.floor((op.min + op.max) / 2) : 0,      // the tab prints this on a running racket
    stage: chain ? chain.stage : 0,
    stageName: chain ? (OPS.FAMILIES.find(f => f.id === op.fam) || {}).stages?.[chain.stage - 1] : null,
    flavour: OPS.flavour(op),
    canAfford: (p.money || 0) >= op.cash && (p.energy || 0) >= op.energy && (p.nerve || 0) >= op.nerve
  };
}
function opsView(p, q) {
  q = q || {};
  const o = ops(p);
  const famId = OPS.FAMILIES.some(f => f.id === q.fam) ? q.fam : OPS.FAMILIES[0].id;
  const all = OPS.byFamily(famId);
  const needle = String(q.q || '').trim().toLowerCase();
  const dFilt = OPS.DISTRICTS.some(d => d.id === q.district) ? q.district : '';
  const gFilt = OPS.GRADES.some(g => String(g.i) === String(q.grade)) ? String(q.grade) : '';
  const matched = all.filter(op =>
    (!dFilt || op.district === dFilt) &&
    (gFilt === '' || String(op.grade) === gFilt) &&
    (!needle || (op.name + ' ' + op.catName + ' ' + op.line + ' ' + op.blurb + ' ' + op.roman).toLowerCase().includes(needle))
  );
  const limit = clampN(parseInt(q.limit, 10) || 24, 6, 60);
  const offset = M.max(0, parseInt(q.offset, 10) || 0);
  const page = matched.slice(offset, offset + limit);
  const rackets = Object.keys(o.rackets).map(id => opRacket(p, OPS.byId(id))).filter(Boolean);
  return {
    total: OPS.TOTAL, perFamily: OPS.PER_FAMILY,
    families: OPS.FAMILIES.map(f => ({ id: f.id, name: f.name, ico: f.ico, count: OPS.byFamily(f.id).length, blurb: f.blurb, verb: f.verb, special: f.special, stages: f.stages || null })),
    fam: famId,
    districts: OPS.DISTRICTS.map(d => ({ id: d.id, name: d.name, tier: d.tier })),
    grades: OPS.GRADES.map(g => ({ i: g.i, roman: g.roman, name: g.name })),
    filters: { q: String(q.q || ''), district: dFilt, grade: gFilt },
    matched: matched.length, shown: page.length, offset, limit,
    list: page.map(op => opView(p, op)),
    rackets,
    readyRackets: rackets.filter(r => r.value > 0).length,
    racketIncome: rackets.reduce((a, r) => a + r.value, 0),
    art: o.art.map((piece, idx) => ({ idx, ...(OPS.byId(piece.id) || {}), paid: piece.paid, value: opArtValue(piece), at: piece.at })),
    artValue: o.art.reduce((a, piece) => a + opArtValue(piece), 0),
    artCap: (OPS.FAMILIES.find(f => f.special === 'art') || {}).vault || 12,
    docs: Object.keys(o.docs).map(id => ({ id, n: o.docs[id], ...(OPS.byId(id) || {}) })),
    chains: Object.keys(o.chains).map(id => {
      const op = OPS.byId(id); const c = o.chains[id];
      const f = OPS.FAMILIES.find(x => x.id === (op || {}).fam) || {};
      return op ? { id, name: op.name, ico: op.ico, stage: c.stage, stageName: (f.stages || [])[c.stage - 1], quality: c.quality, roman: op.roman, districtName: op.districtName } : null;
    }).filter(Boolean),
    stats: { total: o.total, wins: o.wins, fails: o.fails, earned: o.earned, spent: o.spent, fam: o.fam },
    // the sirens on your back: heat drags on every job, so the book shows it
    heat: heatOf(p)
  };
}

// ---------- shared bookkeeping for a single attempt
function opSpend(p, accId, op, o, cashOverride) {
  const cash = cashOverride == null ? op.cash : cashOverride;
  p.money -= cash;
  p.energy -= op.energy;
  if (op.nerve) p.nerve -= op.nerve;
  o.total++; o.spent += cash;
  o.fam[op.fam] = (o.fam[op.fam] || 0) + 1;
  return cash;
}
function opPayout(p, o, op, mult) {
  const pay = opPay(p, op, mult);
  p.money += pay;
  p.reputation = (p.reputation || 0) + op.rep;
  o.earned += pay;
  E.gainXp(p, op.xp);
  return pay;
}
function opDo(accId, opId, body) {
  body = body || {};
  const p = W.ready(W.load(accId)); const o = ops(p);
  const op = OPS.byId(String(opId || ''));
  if (!op) return err('No such operation.');
  // every family shuts down in a cell or a ward — except the back-alley clinic, which is
  // precisely how somebody gets out of a ward in the first place
  const L = locked(p);
  if (L && !(op.special === 'clinic' && inHosp(p) && !inJail(p))) return L;
  const lock = opLocked(p, op); if (lock) return err(lock);
  const cool = opCooldown(p, op.id);
  if (cool > 0) return err(`That one is still settling — ${M.ceil(cool / MIN)} minutes.`);
  if ((p.nerve || 0) < op.nerve) return err(`Too rattled — this needs ${op.nerve} nerve.`);
  if ((p.energy || 0) < op.energy) return err(`Too tired — this needs ${op.energy} energy.`);
  if ((p.money || 0) < op.cash) return err(`You need $${op.cash.toLocaleString()} up front.`);
  const fam = OPS.FAMILIES.find(f => f.id === op.fam);
  let res;
  switch (op.special) {
    case 'racket':   res = opRacketSet(p, o, op); break;
    case 'chain':    res = opChain(p, o, op, body); break;
    case 'run':      res = opRun(p, o, op); break;
    case 'forgery':  res = opForge(p, o, op); break;
    case 'muscle':   res = opMuscle(p, o, op); break;
    case 'chop':     res = opChop(p, o, op, body); break;
    case 'cyber':    res = opCyber(p, o, op); break;
    case 'clinic':   res = opClinic(p, o, op); break;
    case 'art':      res = opArt(p, o, op); break;
    case 'race':     res = opRace(p, o, op, body); break;
    default:         res = err('That family is not open yet.');
  }
  if (res && res.err) return res;
  res.fam = fam.name; res.famIco = fam.ico; res.op = { id: op.id, name: op.name, roman: op.roman, gradeName: op.gradeName, districtName: op.districtName };
  W.save(accId, p);
  return { ok: true, p: W.publicView(p), res };
}

// ---------- 🏦 Rackets: set one up, it earns while you are elsewhere
function opRacketSet(p, o, op) {
  const cost = opSpend(p, null, op, o);
  o.rackets[op.id] = { at: now() };
  const mid = M.floor((op.min + op.max) / 2);
  return { win: true, kind: 'racket', text: `${op.name} is open. It pays about $${mid.toLocaleString()} every ${op.hours}h, banking up for ${op.capHours}h before somebody notices.`, cost, ban: false };
}
function opCollect(accId) {
  const p = W.ready(W.load(accId)); const o = ops(p);
  const L = locked(p); if (L) return L;
  const ids = Object.keys(o.rackets);
  if (!ids.length) return err('You have no rackets running.');
  const lines = []; let paid = 0, lost = 0;
  const heat = heatOf(p);
  for (const id of ids) {
    const op = OPS.byId(id); if (!op) { delete o.rackets[id]; continue; }
    const r = opRacket(p, op);
    if (!r || r.value <= 0) { lines.push(`${op.name}: nothing in the envelope yet.`); continue; }
    // a racket sitting full in the open is a racket the law eventually finds
    const raid = Math.random() * 100 < (2 + op.grade * 0.8 + heat * 0.35);
    if (raid) {
      delete o.rackets[id]; lost++;
      opHeat(p, 6 + op.grade);
      lines.push(`${op.name}: raided. The takings are gone and the block is hot.`);
      continue;
    }
    const pay = M.round(r.value * (dials().payout / 100) * (1 + tipPct(p, 'payoff') / 100));
    p.money += pay; paid += pay; o.earned += pay; o.wins++;
    o.rackets[id] = { at: now() };
    opHeat(p, 1);
    lines.push(`${op.name}: $${pay.toLocaleString()} collected.`);
  }
  if (paid) E.gainXp(p, 6 + lost * 2);
  W.save(accId, p);
  return { ok: true, p: W.publicView(p), res: { kind: 'collect', paid, lost, lines, text: paid ? `The envelopes come in — $${paid.toLocaleString()}.` : 'Nothing worth collecting.' } };
}

// ---------- 💣 Heists: three stages, each one earned
function opChain(p, o, op, body) {
  const f = OPS.FAMILIES.find(x => x.id === op.fam);
  const c = o.chains[op.id] || { stage: 0, quality: 0 };
  const stage = c.stage + 1;
  const scoutCost = op.cash, crewCost = M.round(op.cash * 1.6);
  if (stage === 2 && p.money < crewCost) return err(`The crew wants $${crewCost.toLocaleString()} before they stand up.`);
  const cost = opSpend(p, null, op, o, stage === 1 ? scoutCost : stage === 2 ? crewCost : 0);
  if (stage < 3) {
    const ok = Math.random() * 100 < opChance(p, op) + 6;
    if (ok) c.quality += 1 + (stage === 1 ? 1 : 0);
    else c.quality = M.max(0, c.quality - 1);
    c.stage = stage;
    o.chains[op.id] = c;
    opsSetCool(p, op.id, 1);
    return { ok: true, kind: 'chain', stage, cost, win: ok,
      text: ok
        ? (stage === 1 ? `${op.name}: the walk-through is clean. Two ways in, one camera that has been broken for a month.`
                       : `${op.name}: the crew is assembled and everybody knows their corner.`)
        : (stage === 1 ? `${op.name}: somebody clocked you by the gate. You leave with nothing useful.`
                       : `${op.name}: one of the crew does not turn up. You go forward shorthanded.`) };
  }
  // the pull itself
  const bonus = c.quality * 5;
  const chance = opChance(p, op, bonus);
  const win = Math.random() * 100 < chance;
  delete o.chains[op.id];
  opsSetCool(p, op.id, 12);
  if (win) {
    const pay = opPayout(p, o, op, 0.8 + c.quality * 0.1);
    o.wins++; opHeat(p, op.heat);
    return { ok: true, kind: 'chain', stage: 3, cost, win: true, pay, chance, quality: c.quality,
      text: `${op.name}: in and out. $${pay.toLocaleString()} on the seat beside you and nobody follows you home.` };
  }
  o.fails++; opHeat(p, op.heat * 2);
  const tail = opDanger(p, op, 1.4);
  p.life = M.max(1, p.life - (6 + op.grade * 3));
  return { ok: true, kind: 'chain', stage: 3, cost, win: false, chance, quality: c.quality,
    text: `${op.name}: the door does not open. You leave empty and loud.${tail ? ' ' + tail : ''}` };
}

// ---------- 🚚 Smuggling runs
function opRun(p, o, op) {
  const cost = opSpend(p, null, op, o);
  const chance = opChance(p, op);
  opsSetCool(p, op.id, 4);
  const roll = Math.random() * 100;
  if (roll < chance) {
    const pay = opPayout(p, o, op, 1);
    o.wins++; opHeat(p, op.heat);
    return { ok: true, kind: 'run', win: true, pay, chance, cost, text: `${op.name}: the border waves you through. $${pay.toLocaleString()} at the far end.` };
  }
  if (roll < chance + 12) {   // half a load through — expensive, survivable
    const pay = opPayout(p, o, op, 0.5);
    o.wins++; opHeat(p, op.heat * 1.4);
    return { ok: true, kind: 'run', win: true, partial: true, pay, chance, cost, text: `${op.name}: a roadblock eats half the load. $${pay.toLocaleString()} survives, along with a description of your van.` };
  }
  o.fails++; opHeat(p, op.heat * 2);
  p.life = M.max(1, p.life - (4 + op.grade * 2));
  const tail = opDanger(p, op, 1.2);
  return { ok: true, kind: 'run', win: false, chance, cost, text: `${op.name}: seized at the weighbridge. The load, the money and the night are gone.${tail ? ' ' + tail : ''}` };
}

// ---------- 🖨️ Forgery: papers earn their keep later
function opForge(p, o, op) {
  const cost = opSpend(p, null, op, o);
  const chance = opChance(p, op) + 3;
  opsSetCool(p, op.id, 6);
  if (Math.random() * 100 < chance) {
    o.docs[op.id] = (o.docs[op.id] || 0) + 1;
    o.wins++; opHeat(p, 1);
    const kind = (op.cat === 'notes' || op.cat === 'cheques') ? 'paper' : (op.cat === 'passports' || op.cat === 'badges' || op.cat === 'deeds') ? 'bail' : 'shield';
    return { ok: true, kind: 'forgery', win: true, chance, cost, docKind: kind, n: o.docs[op.id],
      text: `${op.name}: clean off the press. It goes in your coat as a one-shot ${kind === 'paper' ? 'wad of passing paper' : kind === 'bail' ? 'brief for the magistrate' : 'screen from the law'}.` };
  }
  o.fails++; opHeat(p, 2 + op.grade * 0.5);
  return { ok: true, kind: 'forgery', win: false, chance, cost, text: `${op.name}: the press jams and the shop smells of hot ink. You leave with nothing.` };
}
function docUse(accId, docId) {
  const p = W.ready(W.load(accId)); const o = ops(p);
  const op = OPS.byId(String(docId || ''));
  if (!op || op.special !== 'forgery') return err('No such document.');
  if (!o.docs[op.id]) return err('You are not holding one of those.');
  o.docs[op.id]--; if (!o.docs[op.id]) delete o.docs[op.id];
  const kind = (op.cat === 'notes' || op.cat === 'cheques') ? 'paper' : (op.cat === 'passports' || op.cat === 'badges' || op.cat === 'deeds') ? 'bail' : 'shield';
  let text;
  if (kind === 'paper') { const pay = M.round(op.min * 0.6 * (dials().payout / 100)); p.money += pay; o.earned += pay; text = `${op.name} passed at the counter — $${pay.toLocaleString()} in your pocket.`; }
  else if (kind === 'bail') { opTip(p, 'bail', 18 + op.grade * 2, 120, op.name); text = `${op.name} shown to the wrong desk. Bail gets cheaper for the next two hours.`; }
  else { coolHeat(p, 10 + op.grade * 2); text = `${op.name} burned in a bin behind the station. The tail on you goes quiet.`; }
  W.save(accId, p);
  return { ok: true, p: W.publicView(p), res: { kind: 'doc', win: true, text } };
}

// ---------- 🥊 Muscle work
function opMuscle(p, o, op) {
  const cost = opSpend(p, null, op, o);
  const chance = opChance(p, op);
  opsSetCool(p, op.id, 5);
  if (Math.random() * 100 < chance) {
    const pay = opPayout(p, o, op, 1);
    o.wins++; opHeat(p, op.heat);
    const strength = 8 + op.grade * 2;
    opTip(p, 'muscle', strength, (op.buffHours || 3) * 60, op.name);
    return { ok: true, kind: 'muscle', win: true, pay, chance, cost, buff: strength, text: `${op.name}: settled without a word raised. $${pay.toLocaleString()}, and your hands feel like an argument for the rest of the night.` };
  }
  o.fails++; opHeat(p, op.heat * 1.5);
  p.life = M.max(1, p.life - (8 + op.grade * 4));
  const tail = opDanger(p, op, 1.1);
  return { ok: true, kind: 'muscle', win: false, chance, cost, text: `${op.name}: the other side came prepared. You lose the night and a shirt.${tail ? ' ' + tail : ''}` };
}

// ---------- 🔧 Chop shop
function opChop(p, o, op, body) {
  const s = sys(p);
  const cars = s.cars || [];
  const idx = Number.isFinite(body.idx) ? body.idx : cars.map((c, i) => ({ c, i, r: carRating(c) })).filter(x => x.r >= op.carMin).sort((a, b) => a.r - b.r)[0]?.i;
  if (idx == null || !cars[idx]) return err(`The yard wants a machine rated ${op.carMin} or better. Nothing you own fits.`);
  const owned = cars[idx];
  if (carRating(owned) < op.carMin) return err(`That car rates ${M.round(carRating(owned))} — the yard needs ${op.carMin}.`);
  const cost = opSpend(p, null, op, o);
  s.cars.splice(idx, 1);
  opsSetCool(p, op.id, 8);
  const chance = opChance(p, op) + 4;
  const win = Math.random() * 100 < chance;
  const pay = win ? opPayout(p, o, op, 1) : M.round(op.min * 0.35 * (dials().payout / 100));
  if (win) { o.wins++; } else { o.fails++; p.money += pay; o.earned += pay; }
  opHeat(p, op.heat);
  return { ok: true, kind: 'chop', win, pay, chance, cost, car: owned.id,
    text: win
      ? `${op.name}: the ${owned.id} comes apart like it was designed for it. $${pay.toLocaleString()} in parts and scrap.`
      : `${op.name}: you cut too deep and ruin the good half. $${pay.toLocaleString()} and a lot of swearing.` };
}

// ---------- 💻 Cyber jobs
function opCyber(p, o, op) {
  const cost = opSpend(p, null, op, o);
  const chance = opChance(p, op);
  opsSetCool(p, op.id, 7);
  if (Math.random() * 100 < chance) {
    const pay = opPayout(p, o, op, 1);
    o.wins++;
    let extra = '';
    if (['records', 'cameras', 'traffic', 'grid'].includes(op.cat)) {
      coolHeat(p, 6 + op.grade);
      extra = ' The trail behind you gets shorter.';
    } else if (['ledgers', 'boards'].includes(op.cat)) {
      const strength = 6 + op.grade * 2;
      opTip(p, 'edge', strength, 120, op.name);
      extra = ` Everything you read makes the next job easier (+${strength}%).`;
    } else {
      p.energy = M.min(p.max_energy || (p.energy + 1), p.energy + 6 + op.grade * 2);
      extra = ' The job runs itself and you get some sleep.';
    }
    opHeat(p, 1);
    return { ok: true, kind: 'cyber', win: true, pay, chance, cost, text: `${op.name}: in, out, and $${pay.toLocaleString()} moved somewhere with better lawyers.${extra}` };
  }
  o.fails++; opHeat(p, op.heat * 1.6);
  const fine = M.round(op.cash * 0.5);
  p.money = M.max(0, p.money - fine);
  return { ok: true, kind: 'cyber', win: false, chance, cost, fine, text: `${op.name}: the honeypot bites and a clean-up bill lands — $${fine.toLocaleString()} to make it go away.` };
}

// ---------- 🩺 Back-alley clinic
function opClinic(p, o, op) {
  const cost = opSpend(p, null, op, o);
  const chance = opChance(p, op) + 4;
  opsSetCool(p, op.id, 4);
  if (Math.random() * 100 < chance) {
    let cap = Number.isFinite(p.max_life) && p.max_life > 0 ? p.max_life : (E.hpCap ? E.hpCap(p) : 0);
    if (!Number.isFinite(cap) || cap <= 0) cap = 1000;
    const heal = M.round(cap * (0.18 + op.grade * 0.06));
    p.life = M.min(cap, (p.life || 1) + heal);
    const discharged = !!(p.hosp_until && p.hosp_until > now());
    p.hosp_until = 0;                      // no forms, no questions, no ward
    const patch = 6 + op.grade * 2;
    opTip(p, 'patch', patch, (op.buffHours || 4) * 60, op.name);
    o.wins++;
    return { ok: true, kind: 'clinic', win: true, heal, chance, cost, text: `${op.name}: patched and out the side door, ${heal.toLocaleString()} back on your feet${discharged ? ' and off the ward' : ''}.` };
  }
  o.fails++; p.life = M.max(1, p.life - 10);
  return { ok: true, kind: 'clinic', win: false, chance, cost, text: `${op.name}: the table is not clean and neither is the work. You leave worse than you arrived.` };
}

// ---------- 🖼️ Art & collectibles
function opArt(p, o, op) {
  const cap = (OPS.FAMILIES.find(f => f.special === 'art') || {}).vault || 12;
  if (o.art.length >= cap) return err(`The vault takes ${cap} pieces. Move something before you buy again.`);
  const cost = opSpend(p, null, op, o);
  const chance = opChance(p, op) + 2;
  opsSetCool(p, op.id, 2);
  if (Math.random() * 100 < chance) {
    o.art.push({ id: op.id, at: now(), paid: cost });
    o.wins++;
    return { ok: true, kind: 'art', win: true, chance, cost, n: o.art.length,
      text: `${op.name} changes hands quietly for $${cost.toLocaleString()}. It will be worth more next week than it is tonight.` };
  }
  o.fails++; opHeat(p, 2 + op.grade * 0.5);
  const salvage = M.round(cost * 0.4);
  p.money += salvage; o.earned += salvage;
  return { ok: true, kind: 'art', win: false, chance, cost, salvage,
    text: `${op.name}: the seller already had a buyer. You are out of pocket and holding $${salvage.toLocaleString()} worth of nothing.` };
}
function opSell(accId, idx) {
  const p = W.ready(W.load(accId)); const o = ops(p);
  const L = locked(p); if (L) return L;
  idx = parseInt(idx, 10);
  const piece = Number.isFinite(idx) ? o.art[idx] : null;
  if (!piece) return err('Nothing in the vault at that spot.');
  const op = OPS.byId(piece.id);
  const value = opArtValue(piece);
  const heat = heatOf(p);
  const chance = clampN(96 - heat * 0.2 + tipPct(p, 'fence'), 40, 99);
  o.art.splice(idx, 1);
  opsSetCool(p, piece.id, 2);
  if (Math.random() * 100 < chance) {
    const pay = M.round(value * (dials().payout / 100) * (1 + tipPct(p, 'payoff') / 100));
    p.money += pay; o.earned += pay; o.wins++;
    E.gainXp(p, 8 + op.grade * 4);
    W.save(accId, p);
    return { ok: true, p: W.publicView(p), res: { kind: 'sell', win: true, pay, value, text: `${op.name} goes to a collector for $${pay.toLocaleString()} — $${(pay - piece.paid).toLocaleString()} on what you paid.` } };
  }
  const low = M.round(value * 0.55);
  p.money += low; o.earned += low; opHeat(p, 2);
  W.save(accId, p);
  return { ok: true, p: W.publicView(p), res: { kind: 'sell', win: false, pay: low, value, text: `The only buyer in the room knows you are stuck. ${op.name} goes for $${low.toLocaleString()}.` } };
}

// ---------- 🏁 Street circuits
function opRace(p, o, op, body) {
  const s = sys(p);
  const cars = s.cars || [];
  const idx = Number.isFinite(body.idx) ? body.idx : cars.map((c, i) => ({ i, r: carRating(c) })).sort((a, b) => b.r - a.r)[0]?.i;
  if (idx == null || !cars[idx]) return err('You need a car in the garage to enter a circuit.');
  const owned = cars[idx];
  const rating = carRating(owned);
  if (rating < op.carMin) return err(`The field runs cars rated ${op.carMin}+. Yours rates ${M.round(rating)}.`);
  const cost = opSpend(p, null, op, o);
  const edge = clampN((rating - op.carMin) / op.carMin, -0.35, 0.4);
  const chance = opChance(p, op, M.round(edge * 45));
  opsSetCool(p, op.id, 10);
  if (Math.random() * 100 < chance) {
    const pay = opPayout(p, o, op, 1 + M.max(0, edge));
    o.wins++; opHeat(p, op.heat);
    return { ok: true, kind: 'race', win: true, pay, chance, cost, car: owned.id,
      text: `${op.name}: you take it on the last corner and hold it. $${pay.toLocaleString()} and a crowd that will remember the plate.` };
  }
  o.fails++; opHeat(p, op.heat * 1.3);
  const repair = M.round(op.min * 0.25);
  p.money = M.max(0, p.money - repair);
  p.life = M.max(1, p.life - (3 + op.grade));
  return { ok: true, kind: 'race', win: false, chance, cost, repair, car: owned.id,
    text: `${op.name}: beaten on the straight and $${repair.toLocaleString()} of bodywork to explain away.` };
}
...
function panel(accId) {
  ...
  return {
    ...
    ops: (() => {
      const o = ops(p);
      const rackets = Object.keys(o.rackets).length;
      const ready = Object.keys(o.rackets).filter(id => { const op = OPS.byId(id); const r = op && opRacket(p, op); return r && r.value > 0; }).length;
      return { total: OPS.TOTAL, perFamily: OPS.PER_FAMILY, rackets, ready, art: o.art.length, docs: Object.values(o.docs).reduce((a, b) => a + b, 0), chains: Object.keys(o.chains).length, finished: o.total, wins: o.wins, earned: o.earned, spent: o.spent };
    })(),
    tips: tipActive(p),
...
function track(accId, actionName, out) {
  try {
    ...
    if (actionName === 'crime' && out && out.res) { try { addHeat(p, 4); dirty = true; } catch (_) {} }
