...
function opChance(p, op, extra) {
  ...
  const heat = SL ? (SL.ensure(p).heat || 0) : 0;
...
function opHeat(p, n) { if (SL) SL.addHeat(p, M.max(1, M.round(n * (dials().danger / 100)))); }
...
function docUse(accId, docId) {
  ...
  else { if (SL) { const L = SL.ensure(p); L.heat = M.max(0, L.heat - (10 + op.grade * 2)); L.heatAt = now(); } text = `${op.name} burned in a bin behind the station. The tail on you goes quiet.`; }
  W.save(accId, p);
...
function opCyber(p, o, op) {
  ...
  if (Math.random() * 100 < chance) {
    ...
    if (['records', 'cameras', 'traffic', 'grid'].includes(op.cat)) {
      if (SL) { const L = SL.ensure(p); L.heat = M.max(0, L.heat - (6 + op.grade)); L.heatAt = now(); }
