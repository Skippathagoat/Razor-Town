...
function prisonBailCost(p, now) {
  ...
  return Math.round((400 + minsLeft * 22) * (1 + Math.min(2, (p.level || 1) / 40)));
}
...
// ---------------------------------------------------------------- THE DAILY STRIKE — your cut for showing up
function todayKey(now) { return new Date(now).toISOString().slice(0, 10); }
function dailyPay(level, streak) { return 400 + 300 * Math.min(9, Math.max(0, streak - 1)) + (level || 1) * 25; }
function dailyClaim(accId) {
  ...
  if (p.daily.day === today) return { err: 'Already collected today — the desk resets at midnight.' };
  const yesterday = todayKey(now - 24 * 3600 * 1000);
  p.daily.streak = (p.daily.day === yesterday) ? (p.daily.streak || 0) + 1 : 1;
  p.daily.day = today;
  const pay = dailyPay(p.level, p.daily.streak);
  p.money = (p.money || 0) + pay;
  p.happy = Math.min(p.max_happy || 100, (p.happy || 50) + 20);
  E.gainXp(p, 2);
  save(accId, p);
  if (p.daily.streak >= 3) logNews('daily', '\u2728', p.name + ' kept a ' + p.daily.streak + '-day streak alive at the wire desk.');
  return { ok: true, res: { streak: p.daily.streak, pay, text: 'Day ' + p.daily.streak + ' on the wire: +$' + pay.toLocaleString() + ' and the paperwork forgot your name again.' }, p };
}
...
const WHEEL_SEGS = [
  ...
  { label: '$2,000',       w: 20, kind: 'cash',   amount: 2000 },
  { label: '$5,000',       w: 16, kind: 'cash',   amount: 5000 },
  { label: 'CIGS ×10',     w: 12, kind: 'cigs',   amount: 10 },
  { label: '$12,000',      w: 10, kind: 'cash',   amount: 12000 },
  { label: 'ENERGY +30',   w: 8,  kind: 'energy', amount: 30 },
  { label: '$25,000',      w: 6,  kind: 'cash',   amount: 25000 },
  { label: 'JACKPOT $150k', w: 2, kind: 'cash',   amount: 150000 },
];
function wheelMult(streak) { return 1 + 0.12 * Math.min(13, Math.max(0, streak - 1)); }
...
function goJail(p, minutes) {
  minutes = Math.max(1, Math.round(minutes * (1 - 0.15 * (factionUpgrades(p.faction).dentist ? 1 : 0))));
...
function doCrime(accId, crimeId) {
  ...
  const chance = Math.min(0.96, E.crimeSuccessChance(p, crime) * (1 + (crimeBonusPct(p) + subCrime + (fup.muscle ? 6 : 0)) / 100));
...
function stockFeePct(p) { return 0.01 * E.subFeeMult(p); }
...
function joinFaction(accId, fid) {
  ...
  const d = JSON.parse(f.json);
  const cap = C.FACTION_MEMBER_CAP + (d.upgrades && d.upgrades.stash_house ? 10 : 0);
  if (d.memberIds.length >= cap) return { err: 'The roster is full — no more beds until the crew buys stash houses.' };
...
function publicView(p) {
  ...
  return {
    ...
    daily: (() => { const now2 = Date.now(); const st = (p.daily && p.daily.streak) || 0; const same = !!(p.daily && p.daily.day === todayKey(now2));
      const cand = same ? st + 1 : ((p.daily && p.daily.day === todayKey(now2 - 86400000)) ? st + 1 : 1);
      return { streak: st, claimed: same, next: dailyPay(p.level, cand) }; })(),
...
function pawnSell(accId, itemId, qty) {
  ...
  const each = pawnQuote(itemId), take = each * qty;
  p.items[itemId] -= qty; if (p.items[itemId] <= 0) delete p.items[itemId];
  p.money += take;
  save(accId, p);
...
function bustOut(accId, targetId) {
  ...
  p.nerve -= BUST.nerve;
  const fup = factionUpgrades(p.faction);
  const chance = Math.min(0.9, BUST.base + (p.stats.dx || 0) / 2000 + (fup.muscle ? 0.08 : 0) + (p.faction && p.faction === t.faction ? 0.07 : 0));
...
function factionBankIn(accId, amount) {
  ...
  p.money -= amount; fx.d.bank += amount;
  factionSave(p.faction, fx.d); save(accId, p);
  return { p: publicView(p), res: { bank: fx.d.bank } };
}
...
function factionBuyUpgrade(accId, upId) {
  ...
  return { ok: true, res: { id: upId, bank: fx.d.bank } };
}
function factionAnnounce(accId, text) {
  ...
  factionSave(p.faction, fx.d);
  return { ok: true, res: { announce: fx.d.announce } };
}
function factionPromote(accId, targetId) {
  ...
  if (!fx.d.memberIds.includes(targetId)) return { err: 'They are not on the roster.' };
  const i = fx.d.officers.indexOf(targetId);
  if (i >= 0) fx.d.officers.splice(i, 1); else fx.d.officers.push(targetId);
  factionSave(p.faction, fx.d);
  return { ok: true, res: { officers: fx.d.officers } };
}
...
function factionDetail(accId) {
  ...
  const roster = fx.d.memberIds.map(id2 => {
    ...
    return q ? { id: id2, name: q.name, avatar: q.avatar, level: q.level, role: factionRole(fx.d, id2), jailed: !!(q.jail_until && q.jail_until > Date.now()) } : { id: id2, name: 'Ghost', role: 'member' };
  });
  return { faction: {
    id: fx.f.id, name: fx.f.name, tag: fx.f.tag, bank: fx.d.bank, cap: C.FACTION_MEMBER_CAP + (fx.d.upgrades.stash_house ? 10 : 0),
    roster, announce: fx.d.announce, myRole: factionRole(fx.d, accId),
...
module.exports = {
  ...
  factionLoad, factionSave, factionUpgrades, factionBankIn, factionBankOut, factionBuyUpgrade, factionAnnounce, factionPromote, factionDetail,
  doBuy, doSell, doUse,
