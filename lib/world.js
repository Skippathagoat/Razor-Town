...
function load(accId) { return JSON.parse(db().prepare('SELECT json FROM players WHERE acc_id=?').get(accId).json); }
function save(accId, p) {
  db().prepare('UPDATE players SET json=?, name=?, avatar=?, updated_at=? WHERE acc_id=?')
    .run(JSON.stringify(p), p.name, p.avatar, Date.now(), accId);
}
...
function doTrain(accId, stat, gymId) {
  ...
  if (!E.canTrain(p, stat)) return { err: 'Your other skills need work first — balance your training.' };
...
function doAttack(accId, targetId) {
  ...
  if (fight.win) {
    ...
    if (isBot || t.money >= 0) save(targetId, t);
