...
const WEAR = require('./game/wear.js');
const T26 = require('./game/t26.js');   // portraits, travel, tourney, warrants, deals, PDA
...
function save(accId, p) {
  p.avatar = WEAR.lookOf(p);              // the town sees the kit, never a saved face
...
function normalize(p) {
  p.stocks = p.stocks || {}; p.crypto = p.crypto || {};
  p.equip = p.equip || { weapon: null, armour: null, wear: [] };
  // one-time move off the old creator avatar: the eight-part spec is dropped, the
  // body it described is kept, and a citizen who owns nothing gets a starter kit
  // on their back (cheap basics, seeded from the name so the town stays varied).
  if (!p.lookgen) {
    p.lookgen = 1;
    if (!p.gender && WEAR.isLegacyLook(p.avatar)) p.gender = WEAR.legacyGender(p.avatar);
    if (!WEAR.sanitize(p.equip.wear).length) {
      p.items = p.items || {};
      p.equip.wear = WEAR.starterKit(p.name || p._acc || 'citizen');
    }
  }
  p.gender = WEAR.gender(p);
  p.equip.wear = WEAR.sanitize(p.equip.wear);
  p.avatar = WEAR.lookOf(p);
  // 2026.5: a face on the door, a place on the line, a pocket data assistant
  T26.ensurePortrait(p);
  if (!C.DISTRICTS.some(d => d.id === String(p.dist))) p.dist = T26.districtOf(p).id;
  if (p.travel_cd == null) p.travel_cd = 0;
  if (!p.travel_visits) p.travel_visits = {};
  if (p.total_trips == null) p.total_trips = 0;
  if (p.pda_uses == null) p.pda_uses = 0;
  if (p.tourney_wins == null) p.tourney_wins = 0;
  if (p.last_seen == null) p.last_seen = Date.now();
...
function dailyClaim(accId) {
  ...
  E.gainXp(p, 2);
  // 2026.5: a full week on the wire earns a merit
  let meritNote = '';
  if (p.daily.streak > 0 && p.daily.streak % 7 === 0) {
    p.merits = (p.merits || 0) + 1; p.merits_earned = (p.merits_earned || 0) + 1;
    meritNote = ' A full week keeps the Wire honest — one merit added.';
  }
  save(accId, p);
  ...
  return {
    ok: true,
    res: { streak: p.daily.streak, pay, gangBonusPct: status.gangBonusPct, resetAt: nextDayAt(now), text: 'Day ' + p.daily.streak + ' on the wire: +$' + pay.toLocaleString() + bonusText + meritNote + '.' },
    p: publicView(p)
  };
}
// ---------------------------------------------------------------- PDA — the pocket data assistant
// Thin, honest wrapper: quick item use counts toward the One-Handed feat.
function pdaUse(accId, itemId) {
  const r = doUse(accId, itemId);
  if (!r || r.err) return r;
  const p = ready(load(accId));
  const a = T26.countPdaUse(p);
  const ach = a ? (Array.isArray(r.ach) ? r.ach.concat(a) : [a]) : (r.ach || []);
  save(accId, p);
  return { ok: true, p: publicView(p), res: r.res, ach };
}
...
function ready(p) {
  ...
  // presence stamp: at most one extra write per ten quiet minutes
  if (p._acc != null && now - (p.last_seen || 0) > 10 * 60000) {
    p.last_seen = now;
    try { save(p._acc, p); } catch (e) {}
  }
  return p;
}
...
function doCrime(accId, crimeId) {
  ...
  if (ok) {
    ...
    // 2026.5: the district you stand in and the city event on the wire bend the score
    const dmod = T26.distMod(p);
    cash = Math.round(cash * (dmod.crime || 1) * (dmod.payout || 1) * T26.eventCrimeMult() * T26.eventPayoutMult());
    ...
    // 2026.5: warrants and crackdowns make the sirens stickier; the Red Mile makes them looser
    const arrestMult = (T26.eventBustMult() * T26.warrantBustMult(p)) / (T26.distMod(p).bust || 1);
    if (Math.random() < E.arrestChance(p) * (dialPct('danger') / 100) * arrestMult) {
      const wm = T26.checkWarrantFeat(p); if (wm) ach.push(wm);
...
function doWork(accId) {
  ...
  p.energy -= 12;
  const pay = Math.round(job.base * (0.85 + Math.random() * 0.3) * (1 + p.level * 0.02) * (T26.distMod(p).work || 1));
...
function attackTargets(accId) {
  ...
  for (const r of rows) {
    ...
    out.push({
      ...
      portrait: q.portrait || '',
      pic: (q.pic && String(q.pic).length <= 12000) ? q.pic : '',
      last_seen: q.last_seen || 0,
      dist: C.DISTRICTS.some(d => d.id === String(q.dist)) ? String(q.dist) : 'glassq',
...
// Torn has one inventory: you put something on and it comes off the shelf. Iron
// and plate ride the two gear slots, clothing rides the wear list.
function equipItem(accId, itemId) {
  const p = ready(load(accId));
  const it = C.ITEMS[itemId];
  if (!it) return { err: 'No such item.' };
  if (((p.items || {})[itemId] || 0) < 1) return { err: 'You are not carrying it.' };
  if (it.type === 'wear' && it.wear) {
    p.items[itemId] -= 1; if (p.items[itemId] <= 0) delete p.items[itemId];
    const seat = it.wear.part + ':' + it.wear.layer;
    const list = p.equip.wear || [];
    const at = list.findIndex(id => { const w = C.ITEMS[id] && C.ITEMS[id].wear; return w && (w.part + ':' + w.layer) === seat; });
    if (at >= 0) { const old = list[at]; p.items[old] = (p.items[old] || 0) + 1; list[at] = itemId; }
    else list.push(itemId);
    p.equip.wear = WEAR.sanitize(list);
    save(accId, p);
    return { ok: true, p: publicView(p), res: { slot: it.wear.part, layer: it.wear.layer, kind: 'wear', text: `${it.name} on.` } };
  }
  if (it.type !== 'gear' || !it.equip) return { err: 'That is not gear.' };
  p.items[itemId] -= 1; if (p.items[itemId] <= 0) delete p.items[itemId];
  ...
  checkGunCabinet(p);
  save(accId, p);
  return { ok: true, p: publicView(p), res: { slot: it.equip.slot, kind: 'gear' } };
}
// strip(slot) takes off iron or plate; strip('wear', itemId) takes off one piece.
function unequipItem(accId, slot, itemId) {
  const p = ready(load(accId));
  if (slot === 'wear' || itemId) {
    const id = String(itemId || '');
    const list = p.equip.wear || [];
    const at = list.indexOf(id);
    if (at < 0) return { err: 'You are not wearing that.' };
    list.splice(at, 1);
    p.equip.wear = list;
    p.items = p.items || {}; p.items[id] = (p.items[id] || 0) + 1;
    save(accId, p);
    return { ok: true, p: publicView(p), res: { text: `${(C.ITEMS[id] || {}).name || 'Piece'} off.` } };
  }
...
function doAttack(accId, targetId) {
  ...
  if (fight.win) {
    ...
    // 2026.5: Kingsway Docks pays 10% more for clean work
    loot = Math.max(1, Math.round(loot * (T26.distMod(p).fight || 1)));
    p.money += loot;
...
// anything that reaches a SQL binding has to be a plain string or number — a junk body
// value used to throw straight out of node:sqlite and 500 the whole request
function safeId(v) {
...
function buyListing(accId, listingId) {
  const id = safeId(listingId);
  if (id == null) return { err: 'That lot is long gone.' };
...
function cancelListing(accId, listingId) {
  const id = safeId(listingId);
  if (id == null) return { err: 'That is not your lot to take down.' };
  const p = ready(load(accId));
  const row = db().prepare('SELECT * FROM listings WHERE id=?').get(id);
...
function auctionBid(accId, auctionId, amount) {
  settleAuctions();
  const aid = safeId(auctionId);
  if (aid == null) return { err: 'That lot is off the block.' };
  const p = ready(load(accId));
  const a = db().prepare('SELECT * FROM auctions WHERE id=? AND settled=0').get(aid);
...
function publicView(p) {
  p = ready(p);
  return {
    acc: 'self', name: p.name, avatar: p.avatar, gender: p.gender, bio: p.bio, pic: p.pic || '', origin: p.origin,
    portrait: p.portrait || '',
    dist: { id: T26.districtOf(p).id, name: T26.districtOf(p).name, icon: T26.districtOf(p).icon, modLabel: (T26.distMod(p).label) || '' },
    warrant: T26.warrantOf(p),
    last_seen: p.last_seen || 0, visits: p.visits || 0,
    tourney_wins: p.tourney_wins || 0, pda_uses: p.pda_uses || 0,
    travel: { trips: p.total_trips || 0, visits: p.travel_visits || {}, cdLeft: Math.max(0, (p.travel_cd || 0) - Date.now()) },
    ...
    job: p.job,
    items: p.items,
    stocks: p.stocks || {}, crypto: p.crypto || {},
    equip: { weapon: p.equip.weapon || null, armour: p.equip.armour || null, wear: (p.equip.wear || []).slice() },
...
function leaderboard(kind) {
  ...
  return top.map((t, i) => { T26.ensurePortrait(t); return { rank: i + 1, id: t.id, name: t.name, avatar: t.avatar, portrait: t.portrait || '', isBot: t.isBot,
...
function shopsView(accId) {
  ...
  const shops = C.SHOPS.map(s => {
    const dealItem = T26.dailyDeal(s.id);
    return {
      id: s.id, icon: s.icon, name: s.name, area: s.area, blurb: s.blurb, deal: dealItem || null,
      stock: s.stock.map(row => {
        const it = C.ITEMS[row.item];
        const bought = ((p.shop_caps.counts[s.id] || {})[row.item]) || 0;
        const pr = T26.shopRowPrice(s.id, row, p);
        return { item: row.item, name: it.name, icon: it.icon, desc: it.desc, base: pr.base,
          price: pr.price, idx: pr.idx, trend: pr.trend, deal: pr.deal, left: Math.max(0, row.qty - bought), max: row.qty };
      })
    };
  });
  save(accId, p);
  return { shops, eventShopMult: T26.eventShopMult(), warrantMult: T26.warrantShopMult(p) };
}
function shopBuy(accId, shopId, itemId) {
  ...
  const it = C.ITEMS[itemId];
  const pr = T26.shopRowPrice(shopId, row, p);
  ...
  const ach = [];
  if (pr.deal) { const a = T26.markDealBuy(p); if (a) ach.push(a); }
  save(accId, p);
  return { p: publicView(p), res: { item: itemId, name: it.name, price, base: pr.base, deal: pr.deal, left: row.qty - bought - 1 }, ach };
}
