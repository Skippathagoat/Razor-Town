...
function dailyBasePay(level, streak) {
  return 400 + 300 * Math.min(9, Math.max(0, streak - 1)) + (level || 1) * 25;
}
...
function joinFaction(accId, fid) {
  ...
  if (factionMembershipPending(accId)) return { err: 'You already have an application with another crew.' };
  ...
  if (fx.d.recruiting === 'apply') return factionApply(accId, fid);
  ...
  factionLog(fx.d, 'joined', p.name + ' joined the crew.');
...
function factionRoll(accId) {
  ...
  const memberPay = 600 + (p.level || 1) * 40 + Math.min(9, Math.max(0, roll.streak - 1)) * 100;
  const chestPay = Math.round((900 + Math.min(9, Math.max(0, roll.streak - 1)) * 150) * (fx.d.upgrades.signal_room ? 1.5 : 1));
...
function factionOperation(accId, opId) {
  ...
  const chance = Math.min(94, op.chance + statEdge + crewEdge + (fx.d.upgrades.lookouts ? 12 : 0));
  const success = Math.random() * 100 < chance;
  let cash = 0, chest = 0, rep = 0;
  if (success) {
    ...
    factionLog(fx.d, 'operation', p.name + ' closed ' + op.name + ' clean.', chest);
  ...
  factionSave(fx.f.id, fx.d); save(accId, p);
  const text = success
    ? op.name + ' lands: $' + cash.toLocaleString() + ' for you and $' + chest.toLocaleString() + ' for the war chest.'
    : op.name + ' went cold. You got away, but the crew got no cut.';
  return { ok: true, p: publicView(p), res: { operation: op.id, success, chance, cash, chest, rep, text } };
}
...
function factionBankIn(accId, amount) {
  ...
  factionLog(fx.d, 'deposit', p.name + ' put cash into the war chest.', amount);
  factionSave(p.faction, fx.d); save(accId, p);
  return { ok: true, p: publicView(p), res: { bank: fx.d.bank, amount, text: '$' + amount.toLocaleString() + ' added to the war chest.' } };
}
...
function factionDetail(accId) {
  ...
  const roster = fx.d.memberIds.map(id2 => {
    ...
    return q ? { id: id2, name: q.name, avatar: q.avatar, level: q.level || 1, role: factionRole(fx.d, id2), jailed: !!(q.jail_until && q.jail_until > now) } : { id: id2, name: 'Missing citizen', role: 'member' };
  });
  const ops = C.FACTION_OPERATIONS.map(op => {
    const nextAt = Math.max(0, Number((p.faction_ops || {})[op.id]) || 0);
    return { ...op, ready: nextAt <= now, nextAt };
  });
