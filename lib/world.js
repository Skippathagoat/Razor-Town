...
function courseTick(p, now) {
  if (p.course && p.course_ends && now >= p.course_ends) {
    ...
    if (c) {
      ...
      logNews('edu', c.icon, `${nm.name} finished ${c.name} at the Digbeth Technical College.`);
    }
  ...
  for (const a of rows) {
    ...
    if (a.bidder_acc) {
      ...
      if (winner) {
        ...
        systemMsg(a.bidder_acc, "Boulton's Auction Rooms: " + a.qty + 'x ' + name + ' came down on your bid of $' + a.cur_bid.toLocaleString() + '. The lot is in your bag, collector.');
      }
      ...
      if (seller) {
        ...
        systemMsg(a.seller_acc, "Boulton's Auction Rooms: hammer down — your " + a.qty + 'x ' + name + ' sold for $' + a.cur_bid.toLocaleString() + rested + ' ($' + (a.cur_bid - fee).toLocaleString() + ' paid out).');
        if (a.cur_bid >= 100000) logNews('market', '🔨', "Boulton's: " + a.qty + 'x ' + name + ' went under the hammer for $' + a.cur_bid.toLocaleString() + '.');
      }
    } else {
      const seller = loadSafe(a.seller_acc);
      if (seller) {
        ...
        save(a.seller_acc, seller);
        systemMsg(a.seller_acc, "Boulton's Auction Rooms: no takers for your " + a.qty + 'x ' + name + ' — the lot walked home with the porter.');
      }
    }
...
function auctionCreate(accId, itemId, qty, minBid, buyout, hours) {
  ...
  if (!(minBid >= AUCTION_MIN_BID)) return { err: "Boulton's will not open a book under $" + AUCTION_MIN_BID + '.' };
...
function auctionBid(accId, auctionId, amount) {
  ...
  if (previous) {
    ...
    if (oldP) {
      ...
      save(previous, oldP);
      systemMsg(previous, "Boulton's Auction Rooms: " + p.name + ' out-nodded you on the ' + a.qty + 'x ' + it.name + ' — your $' + prevAmt.toLocaleString() + ' walks back to your pocket.');
    }
  }
