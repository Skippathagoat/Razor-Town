...
// Mirrors public/js/avatar.js catalog counts (2026 "older rugged" overhaul: SKINS 9,
// FACES 22, HAIRS 25, TOPS 14, ACCENTS 8, BODIES 2) minus one. Spec: skin|face|hair|top|accent|body
const AVATAR_MAX = { skin: 11, face: 27, hair: 29, shirt: 17, accent: 11, body: 1 };
...
function profileFromForm(form) {
  ...
  const pick = (i, max) => {
    const v = p5[i];
    return Number.isFinite(v) ? Math.max(0, Math.min(max, v)) : clampInt(form[['skin', 'face', 'hair', 'shirt', 'accent', 'body'][i]], max);
  };
  const avatar = [
...
module.exports = { createAccount, verifyLogin, createPlayerForAccount, defaultPlayerJson, sanitize, AVATAR_MAX, makeSalt, hashPassword, signToken, verifyToken, setEmail, setPassword, validEmail, emailTaken, loginKey,
};
