...
// Character creation payload from the client.
// Torn-style: you pick a body (male / female / enby) and that is it — what the
// town sees of you is the clothing and armour you go on to equip. The old
// eight-part look string is gone; `avatar` is now derived from the kit
// (see lib/game/wear.js) and never set by the player.
function profileFromForm(form) {
  ...
  const gender = WEAR.GENDERS.includes(g) ? g : 'm';
  const bio = String(form.bio || '').replace(/[<>&]/g, '').slice(0, 120);
  return { name, origin: originId, gender, bio };
}
...
function defaultPlayerJson(profile) {
  ...
  const p = {
    name: profile.name,
    gender: WEAR.gender(profile),
    avatar: '',                            // filled in below from the equipped kit
    bio: profile.bio,
    origin: origin.id,
    stats,
    xp: 60,
  ...
  // equipment: two gear slots (iron & plate) plus the clothing you are wearing.
  // Nobody starts naked — the kit below is already on your back.
  p.equip = { weapon: null, armour: null, wear: WEAR.starterKit(profile.name) };
...
module.exports = { createAccount, verifyLogin, createPlayerForAccount, defaultPlayerJson, sanitize, makeSalt, hashPassword, signToken, verifyToken, setEmail, setPassword, validEmail, emailTaken, loginKey,
};
