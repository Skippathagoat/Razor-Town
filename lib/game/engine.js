...
function gainXp(p, amount) {
  // 2026: XP Overclock Chip doubles gains while live
  const xb = p.boosters && p.boosters.xp;
  if (xb && xb.until && xb.until > Date.now()) amount = Math.round(amount * (xb.mult || 1));
  p.xp = (p.xp || 0) + amount;
...
function crimePayout(crime, level) {
  const lvlBonus = 1 + Math.max(0, level - crime.lvl) * 0.015;
...
function hpCap(p) {
  const base = Number.isFinite(p.max_hp_base) ? p.max_hp_base : 100;
  return Math.floor(base + (p.stats.de || 0) * 10 + (p.level || 1) * 25);
}

// ---- The Wire Pass: subscription buffs, resolved off the player's own fields
function subOn(p, now) { return !!(p.sub_founder || (p.sub_until && p.sub_until > (now || Date.now()))); }
function subFeeMult(p, now) { return subOn(p, now) ? C.WIRE_PASS.buffs.feeMult : 1; }
...
function refill(p, now) {
  ...
  if (p._ref.happy == null) p._ref.happy = now; // no backfill for old saves — the clock starts when they next wake
  ...
  if (de > 0) {
    let gain = 3 + Math.floor(p.level / 4);
    if (subOn(p, now)) gain = Math.round(gain * C.WIRE_PASS.buffs.energyRegenMult); // wire pass keeps the battery topped
    p.energy = Math.min(p.max_energy, p.energy + de * gain); p._ref.energy += de * PERIODS.energy;
  }
  ...
  // happiness climbs back toward the property ceiling on the same 30-minute tick (~3% of the cap)
  let dh = Math.floor((now - p._ref.happy) / PERIODS.energy);
  if (dh > 0 && (p.happy || 0) < p.max_happy) {
    p.happy = Math.min(p.max_happy, (p.happy || 0) + dh * Math.max(2, Math.round((p.max_happy || 100) * 0.03)));
    p._ref.happy += dh * PERIODS.energy;
  }
  // timers
  ...
  for (const k of ['st','de','sp','dx','xp']) if (p.boosters && p.boosters[k] && p.boosters[k].until <= now) delete p.boosters[k];
  return p;
}

function arrestChance(p) { return Math.min(0.35, 0.16 + (p.total_crimes || 0) * 0.0006); }

module.exports = { C, MINUTES, PERIODS, eff, levelFromXp, gainXp, canTrain, trainBlocker, STAT_NAMES, TRAIN_GAP, trainResult, calcTotal, playerBattleStats, botBattleStats, simulateFight, crimeSuccessChance, crimePayout, xpForCrime, hpCap, refill, arrestChance, subOn, subFeeMult };
