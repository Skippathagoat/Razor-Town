...
const FOUNDER_DEFAULTS = {
  ...
  bank: 250000000,
  reputation: 50000000,
  // Torn-style: a body and the kit he never takes off
  gender: 'm',
  wear: ['tee_white', 'shirt_oxford', 'trench_drab', 'chinos_tan', 'boots_dm', 'chain_gold'],
...
function applyFounderSheet(p) {
  ...
  p.gender = FOUNDER_DEFAULTS.gender;
  p.equip = Object.assign({ weapon: null, armour: null, wear: [] }, p.equip);
  p.equip.wear = WEAR.sanitize(FOUNDER_DEFAULTS.wear);
...
function createFounder(opts = {}) {
  ...
  let p = A.defaultPlayerJson({ name, origin: 'street', gender: FOUNDER_DEFAULTS.gender, bio: '' });
...
function resetFounderFresh(opts = {}) {
  ...
  const fresh = A.defaultPlayerJson({
    ...
    gender: cur.gender || FOUNDER_DEFAULTS.gender,
